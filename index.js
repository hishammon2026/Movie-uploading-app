...
mongoose.connect(mongoURI).then(() => console.log("✅ VLC Mode Active")).catch(err => console.log(err));
...
app.get('/', async (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .card { background: #111; border-radius: 8px; overflow: hidden; border: 1px solid #222; position: relative; transition: 0.3s; }
            ...
            .pic-box { width: 100%; aspect-ratio: 2/3; background: #222; overflow: hidden; }
            ...
            .vjs-matrix.video-js { width: 100%; height: 100%; }
            .close-btn { position: absolute; top: 20px; right: 20px; color: #fff; font-size: 35px; background: rgba(0,0,0,0.5); border: none; z-index: 2100; border-radius: 50%; width: 45px; height: 45px; }
    ...
    <body>
        <header>
            <div style="color:var(--gold); font-size: 1.2em; font-weight:bold;">ANIMEZONE VLC PRO 🏴‍☠️</div>
        ...
        <div id="playerOverlay">
            ...
            <video id="v-player" class="video-js vjs-big-play-centered" controls preload="auto" data-setup='{}'></video>
        </div>
        ...
        <script>
            ...
            function add() {
                if(prompt("Password?") !== "hishammonpass") return alert("Wrong!");
                const seriesName = prompt("Name:"), episodeTitle = prompt("Episode:"), pic = prompt("Poster URL:"), url = prompt("Video URL:");
                if(seriesName && url) {
...
app.listen(PORT, () => console.log("VLC Mode Active!"));
