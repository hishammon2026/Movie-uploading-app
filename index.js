...
mongoose.connect(mongoURI).then(() => console.log("✅ AnimeZone Updated")).catch(err => console.log(err));
...
const Anime = mongoose.model('Anime', {
    ...
    pic: String,
    url: String,
    subUrl: String // സബ്ടൈറ്റിൽ ഫയൽ ഉണ്ടെങ്കിൽ നൽകാൻ
});
...
app.get('/', async (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .watched { opacity: 0.5; border-left-color: #555; } /* കണ്ടുതീർത്ത എപ്പിസോഡുകൾക്ക് */
    ...
    <body>
        ...
        <div class="container">
            <div class="series-grid">
                ${seriesList.map(s => {
                    const first = allData.find(i => i.seriesName === s);
        ...
        <script>
            // Plyr സെറ്റിംഗ്‌സ് - മൾട്ടി ഓഡിയോയും സബ്ടൈറ്റിലും സപ്പോർട്ട് ചെയ്യാൻ
            ...
            // കണ്ടു കൊണ്ടിരുന്ന എപ്പിസോഡ് ഓർമ്മിക്കാൻ
            window.onload = () => {
            ...
            function showEpisodes(series) {
                ...
                container.innerHTML = episodes.map(e => {
                    ...
                    return \`<div class="ep-item \${isWatched}" onclick="playEp('\${e.url}', '\${e.episodeTitle}', '\${e._id}', '\${e.subUrl || ""}')">
                        ...
                        <small style="color:#444;" onclick="event.stopPropagation(); deleteItem('\${e._id}')">DEL</small>
                    </div>\`;
                }).join('');
            ...
            function playEp(url, title, id, sub) {
                ...
                // സബ്ടൈറ്റിൽ ട്രാക്ക് ഉണ്ടെങ്കിൽ അത് ചേർക്കുന്നു
                const videoElement = document.querySelector('#plyrPlayer');
                if(sub) {
            ...
            function addAnime() {
                ...
                const seriesName = prompt("Series Name (eg: One Piece)");
                const episodeTitle = prompt("Episode Name (eg: Ep 791)");
...
app.listen(PORT, () => console.log("AnimeZone Pirate Legend Live!"));
